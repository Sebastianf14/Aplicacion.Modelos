...
namespace Aplicacion.MVC.Controllers
{
    ...
    public class AdminUsersController : Controller
    {
        ...
        public async Task<ActionResult> Artists()
        {
            ...
            {
                var artistMusics = Crud<Music>.GetBy("artist", artist.Id);
        ...
        public async Task<ActionResult> Details(int id)
        {
            ...
            if (userRole == "artists")
            {
                var artistMusics = Crud<Music>.GetBy("artist", user.Id);
        ...
        public async Task<ActionResult> Delete(int id)
        {
            ...
            if (userRole == "artists")
            {
                deleteModel.MusicCount = Crud<Music>.GetBy("artist", user.Id).Count;
        ...
        private void CleanupUserRelatedData(int userId)
        {
            try
            {
                ...
                // Limpiar favoritos
                var userFavoriteMusics = Crud<FavoriteMusic>.GetBy("user", userId);
                var userFavoriteArtists = Crud<FavoriteArtist>.GetBy("user", userId);
                ...
                var userNotifications = Crud<Notification>.GetBy("user", userId);
                foreach (var notification in userNotifications)
                {
                    Crud<Notification>.Delete(notification.Id);
                }
            }
            catch (Exception ex)
            {
